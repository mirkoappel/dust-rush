...
test('Gas, Pfeil-Steuersignal und Bremsen wirken unabhängig von Fahrhilfe',()=>{
  ...
  step(r,{forward:true},60);const speed=r.player.speed;assert.ok(speed>9);assert.ok(speed<13,'Sanftere Beschleunigung');
...
test('Boost erhöht Tempo, verbraucht Energie und lädt ohne Boost nach',()=>{
  const r=new Race();r.start();r.mode='racing';r.pads=[];step(r,{forward:true},100);step(r,{forward:true,boost:true},90);
  assert.ok(r.player.speed>28);assert.ok(r.player.speed<=38);assert.ok(r.player.boost<70);const charge=r.player.boost;
  step(r,{brake:true},30);assert.ok(r.player.boost>charge);
});
...
test('Die echte Fahrhilfe schafft drei Runden ohne Teleportieren',()=>{
  const r=new Race();r.start(true);let limit=60*220;while(r.mode!=='finished'&&limit--)r.step(1/60,{});
  assert.equal(r.mode,'finished','Fahrhilfe bleibt auf der Strecke und beendet das Rennen');assert.equal(r.player.lap,3);assert.ok(r.time<180);assert.ok(r.player.respawns<3);
  assert.ok(r.player.lapTimes.every(t=>t>10));
});
test('Rampe hebt den Truck ab und Landung gibt Sprungpunkte',()=>{
  const r=new Race();r.start();r.mode='racing';const c=r.player,ra=r.ramps[0],p=r.track.at(ra.s+ra.length-1,ra.lane);
  Object.assign(c,{x:p.x,z:p.z,s:p.s,heading:p.heading,speed:26});c.projection=r.track.project(c.x,c.z);
  let jumped=false,landed=false;
  for(let i=0;i<200;i++){r.step(1/60,{forward:true});jumped ||=c.air;landed ||=r.events.some(e=>e.type==='land');}
  assert.ok(jumped);assert.ok(landed);assert.ok(c.score>0);
});
test('Neustart setzt Gegner, Hindernisse, Energie und Ergebnis zurück',()=>{
  const r=new Race();r.start();r.mode='racing';r.player.score=123;r.props[0].active=false;r.player.boost=0;r.mode='finished';r.start();
  assert.equal(r.player.score,0);assert.equal(r.player.boost,100);assert.ok(r.props.every(p=>p.active));assert.equal(r.cars.length,6);assert.equal(r.mode,'countdown');
});
