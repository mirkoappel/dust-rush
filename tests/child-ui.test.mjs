...
test('Beide Spielarten haben eigene Bildkarten und zugängliche Namen',()=>{
  assert.equal((page.match(/class="mode-art"/g)||[]).length,2);
...
test('Zusatztexte und Diagnoseoptionen bleiben hinter dem Erwachsenen-Menü',()=>{
  ...
  assert.ok(advanced);for(const id of ['showStats','assist','suspensionDemo','savePicture'])assert.ok(advanced.includes('id="'+id+'"'));
