...
function fixture(cameraOverrides={}){
  const entries={launch:[25,160],baseSpeed:[8,22],acceleration:[50,200],speed:[0,14],duration:[1,10],recharge:[1,8],lag:[0,150],gap:[1,24]};
...
test('Tuning verändert die wirksamen Nitro- und Kamera-Werte sofort und setzt sie zurück',()=>{
  ...
  f.set('baseSpeed',22);f.set('acceleration',150);f.set('speed',14);f.set('duration',8);f.set('recharge',8);f.set('lag',150);f.set('gap',24);
  ...
  assert.equal(f.camera.maxBoostGap,24);
  assert.equal(f.outputs.gap.textContent,'24 m');
...
test('Bereits erzeugte Fahrkamera liest neue Nachlaufwerte ohne Neustart',()=>{
  ...
  try{
    ...
    CAMERA_TUNING.maxBoostGap=24;CAMERA_TUNING.boostFollowFrequency=.1;
    let max=0;
    for(let i=1;i<=240;i++)max=Math.max(max,c.step(1/60,{boosting:true,speed:10+Math.min(i/60*4,8)}).distanceOffset);
    assert.ok(max>baseline&&max<24,{baseline,max});
