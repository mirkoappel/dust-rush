...
export function makeTruckAddons({shadow=true}={}){
  ...
  const accent=new THREE.MeshPhysicalMaterial({color:'#ff6c24',metalness:.20,roughness:.34,clearcoat:.55});
  ...
  let setup=normalizeBuild(),focus='truck',mounts=null,geometryKey=null;

  function buildWing(){
    const height={pickup:.42,buggy:.40,van:.15,hotrod:.35}[setup.body],half=mounts.width,span=Math.max(1.55,half*2+.50);
    ...
    for(const x of [-span/2,span/2])box(wing,.034,.27,.49,paintMaterials.wing,x,height+.025,-.060,.014);
  ...
  function syncVisibility(){
    wing.visible=setup.wing&&focus!=='engine';
    lights.visible=setup.lights&&focus!=='engine';
    pipes.visible=setup.pipes&&isPartAvailable('pipes',setup)&&focus!=='engine';
  }
  function setBuild(value,bodyMounts){
    ...
    if(key!==geometryKey){
      for(const part of [wing,lights,pipes])disposeGeometry(part);
      buildWing();buildLights();buildPipes();
