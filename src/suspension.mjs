...
export function stepSuspension(state,dt,{air=false,ground=[0,0,0,0],contacts=null,acceleration=0,lateralAcceleration=0,crash=0}={},profile=VEHICLE_PHYSICS) {
  ...
  const massRatio=VEHICLE_PHYSICS_DEFAULTS.massKg/profile.massKg;
  const stiffness=profile.suspension.stiffness*massRatio,damping=profile.suspension.damping*massRatio;
  spring(state,'heave','heaveVelocity',air?.06:limit(mean*.7,-.16,.16),78*stiffness,6.4*damping,dt,-.34,.28);
  spring(state,'pitch','pitchVelocity',air?0:limit(-acceleration*.0025,-.105,.11),62*stiffness,7.2*damping,dt,-.17,.17);
  spring(state,'roll','rollVelocity',air?0:limit(lateralAcceleration*.006,-.15,.15),66*stiffness,6.5*damping,dt,-.21,.21);
  state.wheels.forEach((wheel,i)=>{
    spring(wheel,'offset','velocity',air?-.23:limit(ground[i],-.36,.42),(air?95:280)*stiffness,(air?13:23)*damping,dt,-.38,.45);
