...
const PRESETS={
  ...
  body:{yaw:.65,pitch:.25,distance:7.4},
  wheels:{yaw:1.10,pitch:.10,distance:4.6},
  lift:{yaw:1.02,pitch:.10,distance:4.9},
...
export function createInspection({canvas,getWorld,isWorkshop,onChange}){
  let active=false,view=pose('truck'),start=null,pinch=0;
  ...
  const set=value=>{
    active=value;
    if(value)view=pose('truck');
    clearGesture();sync();
  };
  const focus=part=>{
    const target=Object.hasOwn(PRESETS,part)?part:'truck';
    if(!active||target===view.focus)return;
    view=pose(target);clearGesture();sync();
  };
  const overview=()=>{view={...pose('truck'),distance:9.4,near:false};sync();};
  const toggleZoom=()=>{
    const near=!view.near,base=PRESETS[view.focus].distance;
    view={...view,near,distance:near?base:Math.min(12,base*1.45)};
    sync();
  };
  ...
  document.getElementById('closeInspect').addEventListener('click',overview);
  ...
  window.addEventListener('keydown',e=>{
    if(!active||!isWorkshop())return;
    if(e.key==='Escape'){e.preventDefault();overview();return;}
  ...
  return {set,focus,get active(){return active;},get view(){return {...view};}};
}
