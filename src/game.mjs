...
import {DRIVE_TUNING,SPEEDS} from './physics.mjs';
...
const debugTuning=createDebugTuning({panel:$('tuningPanel'),toggle:$('tuningToggle'),resetButton:$('tuningReset'),nitro:NITRO,camera:CAMERA_TUNING,speeds:SPEEDS,drive:DRIVE_TUNING,onOpen:()=>performanceStats.setOpen(false)});
...
function loop(now){
  ...
  if(loaded){
    ...
    events();syncMode();world.horizonRoll=tilt.active?tilt.lastRoll:0;world.sync(dt,accumulator*120,input());
