...
try{best=JSON.parse(localStorage.getItem('dust-rush-best-v1'))||null;}catch{}
function clearInput(){keys.clear();touch.clear();boostPulseUntil=0;document.querySelectorAll('[data-control]').forEach(b=>b.classList.remove('pressed'));}
...
function input(){
  ...
  return {forward:keys.has('ArrowUp')||keys.has('KeyW'),brake:keys.has('ArrowDown')||keys.has('KeyS'),steer:left||right?Number(right)-Number(left):tilt.read(),boost:keys.has('Space')||keys.has('ShiftLeft')||keys.has('ShiftRight')||control('boost')||performance.now()<boostPulseUntil};
}
function updateTilt(){
  document.body.dataset.tilt=tilt.active?'active':'buttons';
  $('tiltToggle').textContent=tilt.enabled?'▣ Handy-Lenkung an':'▣ Handy-Lenkung';
...
function start(){
  ...
  race.start(mobile||$('assist').checked);world.reset();world.setPlayerColor(menuColor);world.cameraInitialized=false;
...
function showFinish(){
  ...
  if(newBest){best={time:race.finishTime,assist:race.assist,date:new Date().toISOString()};try{localStorage.setItem('dust-rush-best-v1',JSON.stringify(best));}catch{}}
...
async function toggleSound(){sound.enabled=!sound.enabled;await sound.init().catch(()=>{});sound.setEnabled(sound.enabled);$('sound').textContent=sound.enabled?'♪ Ton an':'♪ Ton aus';$('sound').setAttribute('aria-pressed',String(sound.enabled));}
...
const drivingKeys=['ArrowUp','ArrowDown','ArrowLeft','ArrowRight','KeyW','KeyA','KeyS','KeyD','Space','ShiftLeft','ShiftRight'];
window.addEventListener('keydown',e=>{
  if(drivingKeys.includes(e.code)&&['racing','countdown'].includes(race.mode)){e.preventDefault();keys.add(e.code);document.body.dataset.lastDrivingKey=e.code;if((e.code==='Space'||e.code.startsWith('Shift'))&&!e.repeat)boostPulseUntil=performance.now()+450;}
  if(e.repeat)return;
...
document.querySelectorAll('[data-control]').forEach(b=>{
  b.addEventListener('pointerdown',e=>{e.preventDefault();b.setPointerCapture(e.pointerId);touch.set(e.pointerId,b.dataset.control);b.classList.add('pressed');if(b.dataset.control==='boost')boostPulseUntil=performance.now()+450;});
...
function updateHUD(now){
  ...
  $('countdown').textContent=race.mode==='countdown'?Math.min(3,Math.ceil(race.countdown)):now<goUntil?'LOS!':'';
...
function events(){
  for(const e of race.events.splice(0)){
    ...
    if(e.type==='crush'&&e.player)toast('KRACH!');
    if(e.type==='land'&&e.distance>25)toast('JIPPIE!');
...
function loop(now){
  ...
  if(loaded){
    ...
    events();syncMode();world.sync(dt);world.render();sound.update(race.player,race.mode,race.time);
