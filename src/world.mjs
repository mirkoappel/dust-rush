...
import {renderPartPreviews} from './ui/part-previews.mjs';
...
export class World {
  constructor(canvas,race) {
    this.race=race;this.track=race.track;this.canvas=canvas;this.clock=0;this.shake=0;this.dustClock=0;this.props=new Map();this.trucks=[];this.sceneryAssets=[];this.smoothedTarget=new V();this.temp=new V();
    ...
    this.environment=makeEnvironment(this.renderer);this.scene.environment=this.environment.reflection;this.scene.background=this.environment.sky;
  ...
  dispose(){window.removeEventListener('resize',this.handleResize);this.scene.traverse(o=>{o.geometry?.dispose();if(o.material)for(const m of Array.isArray(o.material)?o.material:[o.material]){m.map?.dispose();m.dispose();}});this.environment?.dispose();this.renderer.dispose();}
  buildArena(){
  ...
  async load() {
    ...
    [this.template,this.library,this.propLibrary,this.sceneryLibrary]=await Promise.all([modelData,libraryData,worldAssetData,trackAssetData].map(parse));
    for(const [name,transforms] of this.sceneryAssets)instanceAsset(this.scene,this.sceneryLibrary,name,transforms);
    this.buildProps();
    renderPartPreviews(this.library,this.environment.reflection);
    for(let i=0;i<6;i++) {
  ...
  setPlayerBuild(config){
    ...
    t.kit.setStyle(setup.body);t.kit.setEngine(setup.engine);t.kit.setWheels(setup.wheels);
