...
export function combineDrivingInput(keys,stick,tilt=0,actions={}){
  ...
  const forward=keys.has('ArrowUp')||keys.has('KeyW')||keys.has('Space');
  const brake=keys.has('ArrowDown')||keys.has('KeyS')||keys.has('ShiftLeft');
  return {forward:forward?1:stick.forward,brake:brake?1:stick.brake,
    steer:left||right?Number(right)-Number(left):stick.active&&stick.steer!==0?stick.steer:tilt,
    handbrake:keys.has('KeyX')||!!actions.handbrake,nitro:keys.has('KeyC')||!!actions.nitro};
}
