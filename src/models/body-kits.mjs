...
export function installBodyKits(sprung,originalBody,wheels,library,{shadow=false}={}){
  ...
  const decorations=new Map();let decalStyle='stripes';
  ...
  let body='pickup',engine='classic',wheelStyle='standard',focus='truck',colors={body:'#14bdd1',wheels:'#ff6c24',engine:'#ff6c24',decals:'#ff941f'};
  ...
  function clone(name,category){
    ...
    result.traverse(o=>{
      ...
      const tint=m=>{
        if(category!=='body'&&m.name.includes('Orange')){
  ...
  function sync(){
    if(!variants.has(body)){
      const shell=clone('Body_'+body,'body'),decor=prepareBodyDecor(shell,body);
      decorations.set(body,decor);paintGroups.body.push(...decor.materials);
      for(const material of decor.materials)material.color.set(colors.body);
      variants.set(body,shell);root.add(shell);
    }
    decorations.get(body).setStyle(decalStyle);decorations.get(body).setPaint(colors.decals);
  ...
  return {
    ...
    setDecals(value){decalStyle=value;for(const decor of decorations.values())decor.setStyle(value);},
